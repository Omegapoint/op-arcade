  ...
  def draw(self, surface : pygame.Surface):
    if self.active:
      ...
      if self.seconds_left > 2:
        text = "Klara"
        pygame.draw.circle(surface, [255, 0, 0], to_surface_coordinates(Vector2()), 50, 10)
      elif self.seconds_left > 1:
        text = "Färdiga"
        pygame.draw.circle(surface, [255, 255, 0], to_surface_coordinates(Vector2()), 60, 15)
      else:
        text = "Gå!"
        pygame.draw.circle(surface, [0, 255, 0], to_surface_coordinates(Vector2()), 70, 20)
      text_render = font.render(text, False, [255, 255, 255])
      text_rect = text_render.get_rect()
      text_rect.center = to_surface_coordinates(Vector2())
